...
from .__utils.stats import WOE, probability
from .__utils.merge import merge
from .__utils.utils.func import to_ndarray, bin_by_splits
# from .__utils.utils.func import to_ndarray, np_count, bin_by_splits, split_target
# from .__utils.utils.decorator import frame_exclude, select_dtypes
from .__utils.utils.mixin import BinsMixin
# from .__utils.utils.mixin import RulesMixin, BinsMixin
from .__utils.transformer import Transformer

'''
# 迁移至__utils.transformer
...
class OnehotEncoder:
    ...
    onehot编码
    """
    def __init__(self,data,cols,handle_unknown='error'):
        """
        :param data:训练数据
        :param cols:要进行编码的列
        :param handle_unknown:未知值的处理，默认报错
        """
        self.data = data
        self.cols = cols
        self.handle_unknown = handle_unknown
        self.Encoder = OneHotEncoder(handle_unknown = self.handle_unknown)
        # 检查字段类型，若为数值型，则转换为列名_值
        self.data = self.change_colsname(self.data)
        # 初始化编码器，如果只有一维，需要特殊处理
        if self.cols.__len__() == 1:
            self.onehot = self.Encoder.fit(np.array(self.data[self.cols]).reshape(-1,1))
        else:
            self.onehot = self.Encoder.fit(self.data[self.cols])
            
    def change_colsname(self,data):
        # 如果要编码值是数值型，则将其先变成 => 列名_值
        for col in self.cols:
            if data[col].dtype == 'int64':
                data[col] = data[col].apply(lambda x:col+'_'+str(x))
        return data
                
    def transform(self,data_trans):
        """
        :param data:要进行编码的数据
        """
        data_trans = self.change_colsname(data_trans)
                
        if self.cols.__len__() == 1:
            columns = self.onehot.categories_[0].tolist()
            data_trans[columns]= pd.DataFrame(self.onehot.transform(np.array(data_trans[self.cols]).reshape(-1,1)).toarray())
        else:
            columns = list(itertools.chain.from_iterable(self.onehot.categories_))
            data_trans[columns] = pd.DataFrame(self.onehot.transform(data_trans[self.cols]).toarray())
        
        data_trans = data_trans.drop(self.cols,axis = 1)
        return data_trans

if __name__ == '__main__':
    data1 = pd.DataFrame({'x':['female','female','male','male','nan','nan'],
                         'y':[1,0,0,1,0,0]
                         })
    data2 = pd.DataFrame({'x':['nan','male'],
                         'y':[1,0]
                         })

    onehot = OnehotEncoder(data1,['x'],handle_unknown = 'error')
    tmp1 = onehot.transform(data1.copy())
    tmp2 = onehot.transform(data2.copy())

    onehot = OnehotEncoder(data1,data1.columns,handle_unknown = 'error')
    tmp3 = onehot.transform(data1.copy())
    tmp4 = onehot.transform(data2.copy())
